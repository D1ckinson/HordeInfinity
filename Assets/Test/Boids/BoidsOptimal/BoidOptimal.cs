...
{
    public class BoidOptimal : MonoBehaviour
    {
        ...
        private BoidCommanderOptimal _commander;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (_commander != null)
            {
                List<BoidOptimal> neighbors = GetNeighbors();
        ...
        private Vector3 CalculateAcceleration(List<BoidOptimal> neighbors)
        {
        ...
        public BoidOptimal SetCommander(BoidCommanderOptimal commander)
        {
        ...
        private List<BoidOptimal> GetNeighbors()
        {
            IReadOnlyList<BoidOptimal> boids = _commander.GetAllBoids();
            List<BoidOptimal> neighbors = new();

            foreach (BoidOptimal boid in boids)
            {
                if (boid == this)
                {
                    continue;
        ...
        private Vector3 CalculateSeparation(List<BoidOptimal> neighbors)
        {
            if (neighbors.Count == 0)
            {
                return Vector3.zero;
            ...
            foreach (BoidOptimal boid in neighbors)
            {
                float distance = Vector3.Distance(transform.position, boid.transform.position);
        ...
        private Vector3 CalculateAlignment(List<BoidOptimal> neighbors)
        {
            if (neighbors.Count == 0)
            {
            ...
            foreach (BoidOptimal other in neighbors)
            {
        ...
        private Vector3 CalculateCohesion(List<BoidOptimal> neighbors)
        {
            if (neighbors.Count == 0)
            {
                return Vector3.zero;
